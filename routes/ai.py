...
def _friendly_ai_error(error):
    ...
    if status_code == 429 or "rate" in error_name:
        return "The AI service is busy right now. Please wait a moment and try again.", 429
    if status_code in (408, 504) or "timeout" in error_name:
        return "The AI request took too long. Please try again in a moment.", 504
    if "connection" in error_name or "network" in error_name or "api" in error_name:
        return "We could not reach the AI service. Please check your connection and try again.", 503
...
def ai_improve():
    ...
    return jsonify({"response": result, "html": rendered_html})
